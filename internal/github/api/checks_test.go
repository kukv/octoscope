...
func TestRerunScopePicksTheEndpoint(t *testing.T) {
	...
	for _, tc := range []struct {
		name  string
		scope github.RerunScope
		want  string
	}{
		{"all", github.RerunAll, "/repos/kukv/octoscope/actions/runs/61/rerun"},
		{"failed", github.RerunFailed, "/repos/kukv/octoscope/actions/runs/61/rerun-failed-jobs"},
	} {
...
func TestARerunIsNotRepeatedOnATransientFailure(t *testing.T) {
	...
	if err := c.RerunWorkflow(context.Background(), "kukv/octoscope", 61, github.RerunAll); err == nil {
