...
**幅で見た目が変わる段が 3 つを超えるビューは、段ごとに 1 つずつ録る。**
3 つの幅は「どの段も 1 つは録れている」ことを保証するための最低限であって、
段の数が増えたら足す。Work のカンバンは列数が 1 / 2 / 4 の 3 段に分かれるため、
1 列の段を録るために 50 桁を足している（2026-09-14）。
