...
rerun と event の 2 箇所は全幅に広げるのがむしろ誤りなので、現状の見た目を保つ。
理由は 2 つで別である。event は 1 行に並べた 3 つの選択肢のうち 1 語を指すので、
埋めると隣まで塗る。rerun は各選択肢が 1 行を占めるが、枠も列幅も持たない
2 行の確認メニューなので、端末幅いっぱいの帯は周りの素のテキストから浮く。
