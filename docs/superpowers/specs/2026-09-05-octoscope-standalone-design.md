...
**各カードは枠付きのボックス**で、中身は 3 行。1 行目に状態アイコン・番号・
タイトル、2 行目にタイトルの続き、3 行目にリポジトリ名（owner を除いた短名）・
checks の進捗バー・ラベル・経過時間を置く。checks が無い PR は、バーの代わりに
`approved` / `draft` の語を置く。**選択中のカードは枠の色と背景で示す。**
