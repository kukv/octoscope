...
**各カードは枠付きのボックス**で、中身は 4 行。1 行目に状態アイコン・
リポジトリ名（`owner/name`）・番号、2〜3 行目にタイトル、4 行目に checks の進捗バー・
経過時間・ラベルを置く。checks が無い PR は、バーの代わりに `approved` / `draft` の
語を置く。**選択中のカードは枠の色と背景で示す。**

1 行目が入りきらないときは**リポジトリ名を切り詰め、番号は必ず残す**。
番号がそのカードの識別子であり、切れた番号は何も指さない。

**タイトルの行数は 2 で固定する。** 1 行に収まっても 2 行目は空けたままにする。
カードの高さが可変になると、スクロール窓とマウスの当たり判定が「カードの高さで
割る」計算を使えなくなり、短いタイトルが上にいくつあったかで位置がずれる。
タイトルの 2 行目はインデントしない。狭いカードでは桁が惜しい。

画面下部のドロワーは左右 2 段組。左に本文と
`owner/name #番号 · @作成者 · head → base · +追加 −削除 · ラベル`、
右に checks の一覧。`enter` を押さずに中身を読める。
**Repos タブも同じドロワーを使う**（§4.2）。
...
- リストの下に選択中アイテムの要約を **固定の高さで**出す。選択が動くたびに
  表が上下してはならない。**この要約は Work のドロワーそのものである**（§4.1）。
  左に本文と `owner/name #番号 · @作成者 · head → base · +追加 −削除 · ラベル`、
  右に checks の一覧。サイドバーも跨いで画面の全幅に敷き、100 桁未満では
  Work と同じく畳む（上記「端末幅が足りない場合」の 3 番目）
...
`reviewThreads` は全ページ取る。各スレッドの `comments`（50 件）は
ページングしていないので、それを超えるスレッドではコメントが黙って欠ける。
...
1. サイドバーを畳む（Repos / Search / diff。diff は 100 桁未満で畳み、
   現在のファイル名だけをヘッダに残す）
2. Work のカンバンを 2 列にする（120 桁未満）。4 列のままだと 1 枚のカードの内側が
   18 桁ほどにしかならず、1 行目の `owner/name #番号` が読めない
3. ドロワー・詳細ペインを畳む（100 桁未満。Work / Repos）
4. Work タブのカンバンを 1 列ずつのページングに切り替える（80 桁未満）

**カードの枠はどの幅でも外さない。** 列数が幅に追従するので、枠の 2 桁が致命的に
なるほど細い列は生じない。いちばん細いのは 80 桁の 2 列で、1 枚 38 桁・内側 34 桁ある。

Work の列数は 1 / 2 / 4 の 3 段。セクションが 4 つなので、どの段でもページが
きれいに割れる。3 列は割れず、最後のページが 1 列だけになるので入れない。
`h` / `l` はどの段でも 4 列の間を動き、カーソルのいる列を含むページが表示される。

ドロワーのしきい値は列数と別に持つ。結果として 100〜119 桁は「2 列 + ドロワー」になる。
...
| `icons` | グリフの集合（`unicode` / `nerd` / `ascii`、既定は `unicode`） |
...
`--version` の 4 つになった。恒久的な指定は環境変数 `OCTOSCOPE_ICONS` と設定ファイル
（`icons`）のどちらでもでき、優先順は `--icons` → `OCTOSCOPE_ICONS` → 設定ファイルの
`icons` → 自動判定である。**次にフラグを足すときは、設定ファイル側に置けないかを
先に検討する。**
設定項目の名前を `icons` としたのは、真偽値の名前（`nerd_font` のような）では
`ascii` を指定できないためである。
