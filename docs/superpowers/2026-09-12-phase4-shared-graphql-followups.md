...
### 1. 設計 §6 の表が実態と合わなかった（2 点。2026-09-12 に承認を得て反映済み）
...
**このスライスで直さなかった理由:** どちらも設計文書そのものの書き換えが要り、
文書の訂正は利用者の承認事項である。コードを 1 行も変えない Task 5 の範囲では
提案するところまでしかできなかった。

**その後:** 2026-09-12 に承認を得て、§6 の表の訂正（REST 側から
`pr list` / `issue list` / `repo view` を外し、載っていなかった 5 つを足す）と
§10 への「4 は 5 本に割った」を反映した。あわせて §6 に、パリティの正本が
`internal/usecase/usecase.go` の `source` interface であること、`github.com`
のみを相手にすること（`GH_HOST` / `GH_ENTERPRISE_TOKEN` は見ない）も書いた。
...
- **設計 §6 の訂正と §10 への「4 は 5 本に割った」の 1 行は反映済み**
  （2026-09-12 に承認。積み残し 1 番）
