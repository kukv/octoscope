...
**フィールドを数個ずつ assert しない。** 2026-09-13 の Task 2 で、`domain.PR` の 15 フィールドの
うち 10 が無防備だった（`Additions` や `Head` を落としてもテストが通ることをレビューが実証した）。
変換は「フィールドを 1 対 1 で写す」作業なので、写し漏れこそが唯一の失敗の形である。
数個を見るテストは、その失敗の形を見ていない。

したがって各変換タスクは次の形でテストを書く。

1. ワイヤ型の fixture を、**全フィールドに非ゼロの値を入れて**組む
2. 期待する domain 値を、**全フィールドべた書きで**組む
3. `reflect.DeepEqual`（または `go-cmp`）で**まるごと比べる**

こうすると、変換から 1 行落ちればどの行でも落ちる。fixture にゼロ値を残すと、そのフィールドは
「写しても写さなくても同じ」になり、また無防備になる。**ゼロ値を残さない。**

破壊確認も 1 箇所では足りない。**変換の異なる 3 行**を順に壊して、3 回とも落ちることを見る。
