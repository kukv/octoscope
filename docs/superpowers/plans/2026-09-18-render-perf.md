...
`buildLogLines` はステップ見出しを `theme.Heading().Render` まで通した文字列でキャッシュする。
つまりログの見出しの色は、そのログが届いた時点の背景設定で固定される。Task 2 が
「配色はキャッシュしない」を原則にしたのと逆で、そちらはテストで固定してあるのにこちらは
していない。

**今は実害が無い。** `theme.SetDark` を呼ぶのは起動時の `tea.BackgroundColorMsg` 一箇所だけで、
bubbletea も octoscope も DECSET 2031（背景色の変更通知）を有効にしていないため、背景設定は
起動時に一度決まったきり動かない。ログを取りに行く時点では必ず確定している。

将来、背景の変更を実行中に受け取るようにするなら、ここが追従しない。そのときは素のテキストと
見出し行の位置だけをキャッシュし、`visibleLogRows` で色を当てる形にする。
...
- 描画済み行文字列のメモ化
- `theme` 側のキャッシュには自動的な退行検知が無い（キャッシュを外してもテストは通り、ベンチの
  数字を人が見比べるしかない）。checks 側は `TestLogRowsBuiltOnce` が落ちる
