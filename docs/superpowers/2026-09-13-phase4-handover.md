...
`error.unauthenticated` は「`gh auth login` を実行してください」とだけ言っていた。
`api` バックエンドでトークンが拒否されたときにも出る文言なので、`gh` の無い
マシンでは実行できない案内になっていた。両方の手段を並べる形に直した。

**golden では確認していない。** エラー画面は `wrap(m.errText, m.width)`
（`internal/tui/app/render.go`）で任意の幅に折り返す作りで、この文言を描く
golden は存在しない（既存の `TestUnauthenticatedIsTranslated` は
`strings.Contains` で文言の有無だけを見ている）。代わりに一時的な
`//go:build`-tag 付きテストで **80 桁の en と ja を実際に描画して目で見た**。
どちらも 2 行に折り返り、桁は崩れていなかった。そのテストは確認後に削除した。
